<div>
                        ...
                        <tr class="text-gray-700 dark:text-gray-400">
                            <td class="px-4 py-3">
                                <div class="flex items-center text-sm">
                                    <div>
                                        <p class="font-semibold">{{ optional($setting)->id }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-3 text-sm">
                                {{ optional($setting)->return_days }}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                {{ optional($setting)->fine }}
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <button
                                        class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                        aria-label="Edit" wire:click="editSetting({{ optional($setting)->id }})">
    ...
    @if ($updateForm == true)
        ...
        <form action="" wire:submit.prevent="update({{ optional($setting_id) }})">
            <input
